...
export default function Modal({ title, onClose, children, isOpen = true }: ButtonProps) {
  ...
  return createPortal(
    ...
    >
      ...
      >
        <div className="flex justify-between items-center px-5 py-3 rounded-t-md flex-shrink-0">
          <div className="flex items-center w-full gap-[10px]">
            {typeof title === "string" ? (
              <p className={"text-sm font-semibold text-c8"}>{title}</p>
            ) : (
              title
            )}
          </div>
        ...
        <div className={cn("flex flex-col bg-white overflow-hidden flex-1 min-h-0 rounded-b-md")}>
          <div className="flex-1 overflow-y-auto px-[30px] pt-[20px] pb-3">
